...
class ControlPoint(Emitter):
    ...
    control_radius = 5
    color = (1, 0, 0)
    cursor_type = Gdk.Cursor.new_from_name("move")
    alt_cursor_type = Gdk.Cursor.new_from_name("delete")
...
class Curve(object):
    ...
    control_radius = 5
    line_width = 1
    cursor_type = Gdk.Cursor.new_from_name("crosshair")
...
class CurveEditor(Gtk.DrawingArea):
    ...
    _tooltip = _(
        "<b>Click</b> on a curve to add a control point. <b>Drag"
        "</b> a control point to alter its position. <b>Press Ctrl"
        " and Click</b> on a control point to delete it."
    )
    _help_pixbuf = Gtk.IconTheme.get_for_display(Gdk.Display.get_default()).lookup_icon(
        "help-browser", None, 25, 1, Gtk.TextDirection.NONE, 0
    )
    ...
    def __init__(self, model):
        ...
        # connect all necessary events - using GTK4 event controllers
        self.click_controller = Gtk.GestureClick()
        self.click_controller.connect("pressed", self.button_press_handler)
        self.click_controller.connect("released", self.button_release_handler)
        self.add_controller(self.click_controller)

        self.motion_controller = Gtk.EventControllerMotion()
        self.motion_controller.connect("motion", self.motion_notify_handler)
        self.add_controller(self.motion_controller)

        self.set_draw_func(self.draw_handler)
        # Use resize signal for GTK4
        self.connect("resize", self.configure_handler)
    ...
    def _get_pointer(self):
        ...
        return (
            self.scale.transform_event((x - allocation.x, y - allocation.y)),
            alternate,
        )
    ...
    def on_model_updated(self, model, event=None, *args):
        ...
        # draw of this widget
        if event == "curveUpdate" and (
            args[1] == "cmykChanged" or args[1] == "nameChanged"
        ):
            return
    ...
    def query_tooltip_handler(cls, widget, x, y, keyboard_mode, tooltip):
        ...
        if (
            x >= offset
            and x < pixbuf.get_width() + offset
            or y >= offset
            and y < pixbuf.get_height() + offset
        ):
...
if __name__ == "__main__":
    ...
    # Use GTK4 event controller for key events
    key_controller = Gtk.EventControllerKey()
    key_controller.connect("key-pressed", a.key_press_handler)
    key_controller.connect("key-released", a.key_release_handler)
